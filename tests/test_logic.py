...
from log_analyzer.log_parser import parse_logs
from log_analyzer.report import AverageResponseReport, get_report_generator
...
def test_log_parsing(create_log_file):
    ...
    logs = parse_logs([create_log_file], schema=TEST_LOG_SCHEMA)
    assert len(logs) == 4
    assert logs[0]['url'] == '/api/v1/users'

def test_log_parsing_with_date_filter(create_log_file):
    """Тест парсинга с фильтром по дате."""
    logs = parse_logs([create_log_file], schema=TEST_LOG_SCHEMA, filter_date_str="2025-06-22")
    assert len(logs) == 3
    for log in logs:
...
def test_log_parsing_file_not_found():
    ...
    with pytest.raises(FileNotFoundError):
        parse_logs(["non_existent_file.log"], schema=TEST_LOG_SCHEMA)

def test_average_report_generation(create_log_file):
    """Тест логики отчета 'average'."""
    logs = parse_logs([create_log_file], schema=TEST_LOG_SCHEMA)
    report_generator = get_report_generator('average', schema=TEST_LOG_SCHEMA)
...
def test_report_factory():
    ...
    generator = get_report_generator('average', schema=TEST_LOG_SCHEMA)
    assert isinstance(generator, AverageResponseReport)

    with pytest.raises(ValueError):
        get_report_generator('non_existent_report', schema=TEST_LOG_SCHEMA)